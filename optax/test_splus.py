...
#             'normalize_embeddings': FLAGS.model['normalize_embeddings'],
...
#     elif FLAGS.model.use_decay_at_end:
#         lr_schedule_warmup = optax.linear_schedule(0.0, FLAGS.model['lr'], FLAGS.model['warmup'])
#         lr_schedule = lambda x: jnp.where(x > FLAGS.max_steps*0.9, FLAGS.model['lr']*0.1, lr_schedule_warmup(x))
...
#         optimizer = make_adam(b1=FLAGS.model['beta1'], b2=FLAGS.model['beta2'], ema_rate=FLAGS.model['ema_rate'], use_ema=FLAGS.model['use_ema'], ema_bias_correction=FLAGS.model['ema_bias_correction'])
#     elif FLAGS.model.optimizer == 'schedule_free':
#         optimizer = make_schedule_free(b1=FLAGS.model['beta1'], b2=FLAGS.model['beta2'], ema_rate=FLAGS.model['ema_rate'])
...
#         optimizer = make_shampoo(b1=FLAGS.model['beta1'], b2=FLAGS.model['beta2'], shampoo_diagonal=FLAGS.model['shampoo_diagonal'])
...
#                 if len(u.shape) == 2 and not ('TokenEmbedder_0' in path_str):
#                     u = u * (1 / u.shape[0])
#                 else:
#                     u = u * FLAGS.model['splus_constant']
#                 return u
